...
export type CausticsMaterialType = ShaderMaterial & {
...
export type CausticsProjectionMaterialType = MeshNormalMaterial & {
...
export function createNormalMaterial(side: Side = FrontSide) {
...
export const CausticsMaterial = shaderMaterial(
	{
		...
		depthTexture: null as DepthTexture | null,
...
export type CausticsProps = {
	...
	resolution?: number;
	/** Camera position, it will point towards the contents bounds center, default: Vector3(1,1,1) */
	lightSource?: Vector3 | Object3D;
...
export function createCausticsUpdate(
	updateParameters: () => {
		...
		plane: Mesh<PlaneGeometry, InstanceType<typeof CausticsProjectionMaterial>>;
		normalTarget: WebGLRenderTarget;
		normalTargetB: WebGLRenderTarget;
		causticsTarget: WebGLRenderTarget;
		causticsTargetB: WebGLRenderTarget;
	...
	return function update(gl: WebGLRenderer) {
		const {
			params,
			helper,
			camera,
		...
		if (params.frames === Infinity || (params.frames && count++ < params.frames)) {
			if (params.lightSource) {
				if ((params.lightSource as Vector3).isVector3) {
					lightDir.copy(params.lightSource as Vector3).normalize();
				} else {
					lightDir.copy(group.worldToLocal((params.lightSource as Object3D).getWorldPosition(v)).normalize());
				}
			}
