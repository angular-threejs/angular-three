...
import { ElementRef, InjectionToken, Injector, Optional, SkipSelf } from '@angular/core';
import { Subject } from 'rxjs';
import * as THREE from 'three';
import type { NgtDomEvent, NgtEventManager, NgtPointerCaptureTarget, NgtThreeEvent } from './events';
...
function storeFactory(document: Document, injector: Injector, parentStore: NgtSignalStore<NgtState> | null) {
	if (!window) {
	...
	return Object.assign(store, {
		...
		init(
			...
			const setPerformanceCurrent = (current: number) => {
				store.set((state) => ({ performance: { ...state.performance, current } }));
			};
			...
			store.set({
				...
				performance: {
					...
					regress: () => {
						...
						performanceTimeout = setTimeout(() => {
							setPerformanceCurrent(store.get('performance', 'max') || 1);
				...
				previousStore: parentStore as NgtSignalStore<NgtState>,
				internal: {
					active: false,
					priority: 0,
					frames: 0,
					...
					subscribe: (
						...
						const internal = _store.get('internal');
				...
				setEvents: (events: Partial<NgtEventManager<any>>) => {
					store.set((state) => ({ events: { ...state.events, ...events } }));
				},
...
export const [injectNgtStore, provideNgtStore] = createInjectionToken(storeFactory, {
	isRoot: false,
	deps: [DOCUMENT, Injector, [new Optional(), new SkipSelf(), NGT_STORE]],
	token: NGT_STORE,
});
