...
import type { NgtCameraManual, NgtDpr, NgtSize, NgtState } from './store';
...
export function signalStore<State extends object>(
    ...
    const source = signal(initialState as State, options);
    ...
    return {
        ...
        set: (state: State | ((previous: State) => State)) => {
            ...
            source.update((previous) => ({ ...previous, ...updater(previous) }));
        },
        patch: (state: State) => {
            const updater = (previous: State) => {
            ...
            source.update((previous) => ({ ...updater(previous), ...previous }));
        },
        state,
    };
}
