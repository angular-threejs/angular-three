...
export interface NgtBeforeRenderEvent<TObject> {
...
export const NGT_STORE = new InjectionToken<NgtSignalStore<NgtState>>('NGT_STORE');
export const injectStore = createInjectFn(NGT_STORE);

export function provideStore(store?: NgtSignalStore<NgtState>) {
	if (store) return { provide: NGT_STORE, useFactory: () => store };
...
function storeFactory() {
	...
	Object.defineProperty(store, 'pointerMissed$', { value: pointerMissed$ });
