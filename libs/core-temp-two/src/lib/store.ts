...
import { ElementRef, InjectionToken, Optional, SkipSelf } from '@angular/core';
...
import { createInjectionToken, is, makeDpr, signalStore, type NgtSignalStore } from './utils';
...
export type NgtState = {
	...
	previousRoot?: NgtSignalStore<NgtState>;
	/** Internals */
	internal: NgtInternalState;
};

export function ngtStoreFactory(
	{ invalidate, advance }: NgtLoop,
	document: Document,
	parentStore: NgtSignalStore<NgtState> | null,
) {
	...
	const store: NgtSignalStore<NgtState> = signalStore<NgtState>((storeApi) => {
		...
		return {
			...
			invalidate: (frames = 1) => invalidate(storeApi, frames),
			advance: (timestamp: number, runGlobalEffects?: boolean) => advance(timestamp, runGlobalEffects, storeApi),
			...
			previousRoot: parentStore as NgtSignalStore<NgtState>,
			internal: {
				active: false,
				priority: 0,
				frames: 0,
	...
	Object.defineProperty(store, 'pointerMissed$', {
		get: () => pointerMissed$,
	});
...
export type NgtStore = ReturnType<typeof ngtStoreFactory>;
export const NGT_STORE = new InjectionToken<NgtStore>('NgtStore');
export const [injectNgtStore, provideNgtStore] = createInjectionToken(ngtStoreFactory, {
	isRoot: false,
	token: NGT_STORE,
	deps: [NGT_LOOP, DOCUMENT, [new Optional(), new SkipSelf(), NGT_STORE]],
});
